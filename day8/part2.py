...
while 1:
  ...
  for b1 in boxes:
    for b2 in boxes:
      if networks and b1 in networks[0] and b2 in networks[0]:
        continue
  ...
  top_network = sorted(networks, key=lambda n: len(n), reverse=True)[:1]
  print(len(top_network[0]))
  if len(top_network[0]) == len(boxes):
    print(box1['x'] * box2['x'])
    break
